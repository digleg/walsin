...
export default class ListSort extends React.Component {
  ...
  onMouseDown = (i, e) => {
    ...
    const childStyle = this.children.map((item, ii) => {
      ...
      if (cItem) {
        ...
        const parentHeight =
          item.parentNode.clientHeight - parseFloat(getComputedStyle(item.parentNode).getPropertyValue('padding-bottom'));
        const parentWidth = item.parentNode.clientWidth - parseFloat(getComputedStyle(item.parentNode).getPropertyValue('padding-right'));
    ...
    const animation = this.children.map(
      (item, ii) => (i === ii && (!this.props.dragClassName ? { scale: 1.2, boxShadow: '0 10px 10px rgba(0,0,0,0.15)' } : null)) || null
    );
    this.index = i;
    this.swapIndex = i;
    this.mouseXY = {
    ...
    if (this.props.dragClassName) {
      ...
      this.listDom.className = `${this.listDom.className.replace(this.props.dragClassName, '').trim()} ${this.props.dragClassName}`;
    }
    this.isDrage = true;
    this.setState({
      style,
  ...
  onMouseUp = e => {
    ...
    if (this.props.dragClassName) {
      this.listDom.className = `${this.listDom.className.replace(this.props.dragClassName, '').trim()}`;
    }
  ...
  onMouseMove = e => {
    ...
    if (this.props.animType === 'x') {
      ...
      this.swapIndex = childStyle[this.index].top < this.childStyle[this.index].top ? 0 : this.index;
      this.swapIndex =
        childStyle[this.index].top > this.childStyle[this.index].top + this.childStyle[this.index].height
          ? childStyle.length - 1
          : this.swapIndex;
