...
export class TagPage extends React.PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      showFieldArr: '',
      tolFieldArr: '',
      showFieldArrBool: [true, true, true, true, true, true, false, false, false, false, true, false],
      trackEditDiaInfoMac: '',
  ...
  onRowSelection(value) {
    ...
    if (value['loc.name'] !== 'NA') {
      this.props.dispatch(getTrackList({ id: value['information.id'], type: tagType === '' ? '1' : tagType }));
    }
  }
    ...
    return (
      tagTableData.length !== 0 && (
        <Paper style={{ border: '1px solid #ced4da', marginTop: '0.5em', backgroundColor: '#FAFAFA' }}>
          ...
          >
            <FormattedMessage {...messages.workListDataList} />
          </div>
    ...
    return (
      trackCntList !== null && (
        <div style={{ backgroundColor: '#F5F5F5', justifyContent: 'center' }}>
          <BarChart dataInput={trackCntList} />
        </div>
      )
    );
  }
  ...
  devMetaListFunc() {
    ...
    const showFieldArr = [];
    const tolFieldArr = [];
    const formattedMechineHisInfo = intl.formatMessage({ id: 'MERC.containers.TagPage.mechineHisInfo' });
    const formattedEditMechineInfo = intl.formatMessage({ id: 'MERC.containers.TagPage.editMechineInfo' });
        ...
        <Tooltip title={formattedEditMechineInfo} placement="left" classes={{ tooltip: classes.tooltipCus }}>
          <IconButton
            ...
            onClick={() => {
              ...
              // get field in dia
              dispatch(getFilterInTrackEditF({ factory: { fport: 165 } }));
              dispatch(getFilterInTrackEditZ({ searchField: 'factory', val: meta.factory.val, child: 'zone', fport: '165' }));
              dispatch(getFilterInTrackEditS({ searchField: 'zone', val: meta.zone.val, child: 'station', fport: '165' }));
    ...
    return (
      <Paper style={{ border: '1px solid #ced4da', marginTop: '0.5em', backgroundColor: '#FAFAFA' }}>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          ...
          >
            <FormattedMessage {...messages.mechineDataList} />
          </div>
  ...
  filterSearchFunc() {
    ...
    const formattedEndTime = intl.formatMessage({ id: 'MERC.containers.TagPage.endTime' });
    const formattedStartTime = intl.formatMessage({ id: 'MERC.containers.TagPage.startTime' });
    const formattedSearch = intl.formatMessage({ id: 'MERC.containers.TagPage.search' });
    const formattedClearSearch = intl.formatMessage({ id: 'MERC.containers.TagPage.clearSearch' });
    // const formattedWorkListType = intl.formatMessage({ id: 'MERC.containers.TagPage.workListType' });
    const formattedSrhByWorkListID = intl.formatMessage({ id: 'MERC.containers.TagPage.srhByWorkListID' });
    const formattedWorkListID = intl.formatMessage({ id: 'MERC.containers.TagPage.workListID' });
    return (
      <Grid container className={classes.root} justify="center" spacing={8} style={{ marginTop: 5, marginBottom: 5 }}>
  ...
  tagListFunc() {
    ...
    const formattedWorkListHisInfo = intl.formatMessage({ id: 'MERC.containers.TagPage.workListHisInfo' });
    const formattedEditWorkListInfo = intl.formatMessage({ id: 'MERC.containers.TagPage.editWorkListInfo' });
    ...
    const buttonInfoFormatter = (cell, row) => {
      return (
        <Tooltip title="工單資訊" placement="left" classes={{ tooltip: classes.tooltipCus }}>
          <IconButton
            className={classes.button}
            aria-label="Info"
            onClick={() => {
              ...
              const { tagType } = this.state;
              if (row['loc.name'] !== 'NA') {
                this.props.dispatch(getTrackList({ id: row['information.id'], type: tagType === '' ? '1' : tagType }));
              }
            }}
          >
            <InfoIcon />
        ...
        <Tooltip title={formattedWorkListHisInfo} placement="left" classes={{ tooltip: classes.tooltipCus }}>
          <IconButton
            ...
            onClick={() => {
              ...
              this.props.dispatch(getTrackbyIdList({ type: 1, typeId: row['information.id'] }));
            }}
          >
            <HistoryIcon />
          </IconButton>
    ...
    const options = {
      noDataText: (
        <RaisedButton
          label={<FormattedMessage {...messages.newWorklist} />}
          primary
          onClick={() => {
            // insertTag
            let tagSting = '01';
  ...
  showProcTime() {
    ...
    // const formattedWorkListProcessTime = intl.formatMessage({ id: 'MERC.containers.TagPage.workListProcessTime' });
    const formattedShowWorkListDtl = intl.formatMessage({ id: 'MERC.containers.TagPage.showWorkListDtl' });
  ...
  dialogFunc() {
    ...
    const tagHisFunc = () => {
      ...
      const showFieldArr = [
        'loc.name',
        'loc.meta.id',
        'loc.meta.factory.name',
        'loc.meta.station.name',
        'loc.meta.zone.name',
        'in',
        'out',
        'extra.NW',
              ...
              >
                ...
                <Tooltip title={formattedSearch} placement="bottom" classes={{ tooltip: classes.tooltipCus }}>
                  <IconButton
                    ...
                    onClick={() => {
                      this.props.dispatch(
                        getTrackbyIdList({ type: 1, typeId: tagHisDiaInfo['information.id'], date: [dateFromTrack, dateToTrack] })
                      );
                    }}
                  >
                    <SearchIcon />
    ...
    const trackEditFunc = () => {
      ...
      if (editFList !== null && editZList !== null && editSList !== null) {
        for (let i = 0; i < Object.keys(trackEditDiaInfo).length; i += 1) {
          ...
          const value = Object.values(trackEditDiaInfo)[i];
          const editValue = key.concat('Edit');
          const formattedMessageString = 'MERC.containers.TagPage.'.concat(key);
          ...
          if (key === 'factory' || key === 'station' || key === 'zone') {
            mustField.push(
                  ...
                  <SelectField
                    ...
                    onChange={(event, index, valueSelect) => {
                      ...
                      if (editValue === 'zoneEdit') {
                        this.props.dispatch(
                          getFilterInTrackEditS({ searchField: 'zone', val: valueSelect, child: 'station', fport: '165' })
                        );
    ...
    const tagInfoDiaFunc = () => {
      const { trackList } = this.props;
      return (
        trackList.size !== 0 && Object.keys(trackList).length !== 0 && Object.values(trackList.factory).length !== 0 && this.showProcTime()
      );
    };
    ...
    const formatMessageHisInfo = intl.formatMessage({ id: 'MERC.containers.TagPage.hisInfo' });
    const formatMessageEdit = intl.formatMessage({ id: 'MERC.containers.TagPage.edit' });
    const formatMessageInfo = intl.formatMessage({ id: 'MERC.containers.TagPage.info' });
    const formatMessageChooseShowField = intl.formatMessage({ id: 'MERC.containers.TagPage.chooseShowField' });
    const formattedWorkListProcessTime = intl.formatMessage({ id: 'MERC.containers.TagPage.workListProcessTime' });
    return (
      <div>
        <Dialog
  ...
  renderHeaderData(column) {
    ...
    for (let i = 0; i < column.length; i += 1) {
      if (column[i] === 'macAddr') {
        formattedShowFieldString = 'app.containers.TagPage.'.concat(column[i]);
        formattedShowField = intl.formatMessage({ id: formattedShowFieldString });
      } else {
        formattedShowFieldString = 'MERC.containers.TagPage.'.concat(column[i]);
        formattedShowField = intl.formatMessage({ id: formattedShowFieldString });
      }
      if (i === 0) {
  ...
  render() {
    ...
    const formattedMessageMechine = intl.formatMessage({ id: 'MERC.containers.TagPage.mechineData' });
    const formattedMessageWorkList = intl.formatMessage({ id: 'MERC.containers.TagPage.workListData' });
    return (
      <div>
        <Helmet>
          <title>TagPage</title>
          <meta name="description" content="Description of TagPage" />
        </Helmet>
        {searchFilter.size === 0 ? (
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <CircularProgress
...
const withReducer = injectReducer({ key: 'tagPage', reducer });
const withSaga = injectSaga({ key: 'tagPage', saga });
