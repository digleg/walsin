...
export function* getSearchFilterSaga() {
  const respSearchFilter = yield call(api.getSearchFilterByPersonnel);
  console.log('respSearchFilter data:', respSearchFilter);
  if (respSearchFilter.responseCode === '000') {
    ...
    if (respSearchFilter.responseCode === '000') {
      ...
      const trackCntList = [];
      console.log('respSearchOption data:', respSearchOption);
      if (respSearchOption.responseCode === '000') {
        for (let j = 0; j < respSearchOption.option.length; j += 1) {
          ...
          if (respGetTrackCount.responseCode === '000') {
            console.log('respGetTrackCount data:', respGetTrackCount);
            trackCntList.push({
        ...
        // initial meta data
        yield put(getDeviceMeta({ fport: respSearchFilter.field[rootIndex].fport, meta: 'factory_s', val: 'f1' }));
        yield put(getTagByMeta({ type: 'factory_s', meta: 'f1' }));
...
export function* getShowFieldSaga() {
  ...
  const respGetShowField = yield call(api.getZoneShowField, { data: dataItem });
...
export function* getTagTypeSaga() {
  const respGetTabType = yield call(api.getTagTypeIn2);
